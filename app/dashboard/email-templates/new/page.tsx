...
export default function NewEmailTemplatePage() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const { error: insertError } = await supabase.from("email_templates").insert(record);
  ...
  return (
    <div className="max-w-2xl">
      <div className="mb-8">
        ...
        <h1 className="text-2xl font-bold text-gray-900 mt-2">Add Email Template</h1>
      </div>
      ...
      <form onSubmit={handleSubmit} className="card space-y-6">
        ...
        <div>
          ...
          <input id="thumbnail_url" name="thumbnail_url" type="url" className="input" placeholder="Enter thumbnail url" />
        </div>
        <div>
        ...
        <div className="flex items-center gap-3 pt-4 border-t">
          <button type="submit" disabled={loading} className="btn-primary">
            {loading ? "Saving..." : "Create Email Template"}
          </button>
