...
export default function NewSignupFormPage() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const { error: insertError } = await supabase.from("signup_forms").insert(record);
  ...
  return (
    <div className="max-w-2xl">
      <div className="mb-8">
        ...
        <h1 className="text-2xl font-bold text-gray-900 mt-2">Add Signup Form</h1>
      </div>
      ...
      <form onSubmit={handleSubmit} className="card space-y-6">
        ...
        <div>
          ...
          <input id="fields" name="fields" type="text" className="input" placeholder="Enter fields" />
        </div>
        <div>
          <label htmlFor="submit_button_text" className="label">Submit Button Text</label>
          <input id="submit_button_text" name="submit_button_text" type="text" className="input" placeholder="Enter submit button text" />
        </div>
        <div>
          <label htmlFor="success_message" className="label">Success Message</label>
          <input id="success_message" name="success_message" type="text" className="input" placeholder="Enter success message" />
        </div>
        <div>
          <label htmlFor="redirect_url" className="label">Redirect Url</label>
          <input id="redirect_url" name="redirect_url" type="url" className="input" placeholder="Enter redirect url" />
        </div>
        <div>
        ...
        <div>
          ...
          <input id="public_url" name="public_url" type="url" className="input" placeholder="Enter public url" />
        </div>
        <div>
          <label htmlFor="submission_count" className="label">Submission Count</label>
          <input id="submission_count" name="submission_count" type="number" className="input" placeholder="Enter submission count" />
        </div>

        <div className="flex items-center gap-3 pt-4 border-t">
          <button type="submit" disabled={loading} className="btn-primary">
            {loading ? "Saving..." : "Create Signup Form"}
          </button>
